...
class GeneralTrainer(object):
    def __init__(self, model, optimizer_builder, loss_func, lr_schedule_builder=None, score_metric:dict={}, pre_net=None):
        self.model = model
    ...
    def set_tqdm(self, notebook_tqdm=False):
    ...
    def load_data(self, filepath):
        ...
        if data.get('epoch') is not None:
            self.last_epoch = data.get('epoch') + 1
    ...
    def train(self, train_loader, val_loader, test_loader=None, epochs=10, device='cpu', reset=True):
        # initialize
        ...
        for epoch in self.tqdm(range(self.last_epoch, self.last_epoch + epochs), desc="Epoch"):
